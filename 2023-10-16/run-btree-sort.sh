					...
					for r in `seq 1 $NRUNS`; do
						...
						for scan in indexonlyscan indexscan bitmapscan seqscan; do
							...
							if [ "$scan" == "indexonlyscan" ]; then
								...
								query="select a from btree_test where a >= $value order by a limit $matches"
							else
								psql -t -A test -c "vacuum btree_test" >> $LOGFILE 2>&1
								query="select * from btree_test where a >= $value order by a limit $matches"
							fi
							...
							# random value to search for in the table, but we need to keep it
							# somewhat evenly spread through the range to prevent skewed results
...
EOF
							...
							echo "btree-sort" $MACHINE $time $rows $dataset $matches $distinct $build $prefetch $ts $is $scan uncached $r $value $nfound $cost $duration >> $RESULTS 2>&1
...
EOF
							...
							echo "btree-sort" $MACHINE $time $rows $dataset $matches $distinct $build $prefetch $ts $is $scan cached $r $value $nfound $cost $duration >> $RESULTS 2>&1
