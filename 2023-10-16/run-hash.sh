					...
					for r in `seq 1 $NRUNS`; do
						...
						for scan in indexonlyscan indexscan bitmapscan seqscan; do
							...
							if [ "$scan" == "indexonlyscan" ]; then
								...
								relfile=`psql -t -A test -c "select relfilenode from pg_class where relname = 'hash_test'"`
								rm $DATADIR/base/$dboid/${relfile}_vm >> $LOGFILE 2>&1
								query="select a from hash_test where a = $value"
							else
...
EOF
							...
							echo "hash" $MACHINE $time $rows $dataset $matches $distinct $build $prefetch $ts $is $scan uncached $r $value $nfound $cost $duration >> $RESULTS 2>&1
...
EOF
							...
							echo "hash" $MACHINE $time $rows $dataset $matches $distinct $build $prefetch $ts $is $scan cached $r $value $nfound $cost $duration >> $RESULTS 2>&1
