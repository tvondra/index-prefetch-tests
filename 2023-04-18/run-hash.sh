...
for n in 1 10 100 1000 10000 100000; do
	...
	psql test -c "drop table hash_test" >> $LOGFILE 2>&1
...
EOF
		...
		echo "hash" $r $n $q $v master uncached $e >> $RESULTS 2>&1
...
EOF
		...
		echo "hash" $r $n $q $v master cached $e >> $RESULTS 2>&1
...
EOF
		...
		echo "hash" $r $n $q $v patched uncached $e >> $RESULTS 2>&1
...
EOF
		...
		echo "hash" $r $n $q $v patched cached $e >> $RESULTS 2>&1
...
EOF
		...
		echo "hash" $r $n $q $v bitmapscan uncached $e >> $RESULTS 2>&1
...
EOF
		...
		echo "hash" $r $n $q $v bitmapscan cached $e >> $RESULTS 2>&1
...
EOF
		...
		echo "hash" $r $n $q $v seqscan uncached $e >> $RESULTS 2>&1
...
EOF
		...
		echo "hash" $r $n $q $v seqscan cached $e >> $RESULTS 2>&1
