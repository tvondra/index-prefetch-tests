...
main(int argc, char **argv)
{
	...
	printf("preadv2 NOWAIT time %ld us  calls %d  hits %d  misses %d\n",
		   TIME_DELTA(time_start, time_end), ncalls, nhits, nmisses);

	/* now do the same thing, but without the RWF_NOWAIT flag */
	gettimeofday(&time_start, NULL);
	...
	/* print some statistics */
	printf("preadv2 WAIT time %ld us  calls %d  hits %d  misses %d\n",
		   TIME_DELTA(time_start, time_end), ncalls, nhits, nmisses);
}
